...
async function summarizeNews(newsList) {
    ...
    [작성 가이드라인]
    1. **선별**: 반드시 '생성형 AI(LLM, 이미지 생성, 비디오 생성 등)'와 직접 관련된 핵심 뉴스만 5~10개 선정하세요.
    2. **서식**: 각 뉴스는 아래 형식을 따르세요.
       - **[제목]**: 뉴스의 핵심 내용을 한눈에 알 수 있는 제목
       - **[요약]**: 핵심 내용 요약 (불렛 포인트 사용 권장)
       - **[의의]**: 해당 뉴스가 갖는 기술적/산업적 의미
       - **[출처]**: [원문 링크 보기](링크주소) (배포 날짜: YYYY-MM-DD 형식으로 표기)
...
async function selectWeeklyTop10(weeklyNews) {
    ...
    2. **서식**: 
       ...
       - **요약**: 핵심 내용 브리핑
       - **선정 이유**: 전문가적 식견
       - **참고**: [원문 기사 보기](사용된 뉴스 데이터의 제목을 클릭하면 연결되도록 하거나, 본문 내 링크 포함)
