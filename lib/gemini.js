...
async function summarizeNews(newsList) {
    ...
    const prompt = `
    다음은 오늘 수집된 AI 뉴스들입니다. '생성형 AI(Generative AI)'와 밀접하게 연관된 핵심 뉴스들만 선별하여,
    각 뉴스에 대해 핵심 내용을 1~2문장으로 요약하고, 관련 기술적 의의를 간단히 적어주세요.
    생성형 AI와 관련이 적은 일반적인 IT 뉴스는 제외해 주세요.
    한국어로 작성해 주세요.
...
async function selectWeeklyTop10(weeklyNews) {
    ...
    const prompt = `
    다음은 지난 한 주간 수집된 AI 뉴스들입니다. '생성형 AI(Generative AI)' 트렌드를 중심으로
    가장 중요도가 높고 파급력이 큰 뉴스 10개를 선정해서 랭킹순으로 정리해 주세요.
